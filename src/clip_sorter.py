...
def calculate_video_file_linking(video_path: str, similarity_threshold: 5) -> List:
    frame_mapping = extract_end_frames(video_path)
...
def extract_end_frames(video_path: str, video_type="mp4") -> Mapping:
    files = glob.glob(f'{video_path}/*.{video_type}')
    frame_mapping = {}
...
def find_linking(mapping):
    ...
    if valid_linking is False:
        logger.warning("Cannot find linked list")
        return None
