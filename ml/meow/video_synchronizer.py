...
def extract_subclip(input_video_path, start_time, end_time) -> Clip:
    video = VideoFileClip(input_video_path)
    subclip: Clip = video.subclip(start_time, end_time)
    return subclip


# If delay is positive, audio1 needs to be delayed and negative if audio2 needs to be delayed
def synchronize_videos(video1_path, video2_path, delay: float):
    ...
    if delay >= 0:
        ...
        clip1 = extract_subclip(video1_path, video1_start_time, video1_end_time)
        clip2 = extract_subclip(video2_path, video2_start_time, video2_end_time)
