...
from utils.audio_utils import merge_audio_tracks
...
def run_with_args(left_videos: List[str], right_videos: List[str], use_mixer: bool = True,
    ...
    left_video_path = os.path.join(temp_dir, f'{generate_random_string()}.{file_type}')
    right_video_path = os.path.join(temp_dir, f'{generate_random_string()}.{file_type}')
    ...
    left_audio_path = os.path.join(temp_dir, f'{generate_random_string()}.wav')
    right_audio_path = os.path.join(temp_dir, f'{generate_random_string()}.wav')
    ...
    samplerate1, audio1 = wavfile.read(left_audio_path)
    samplerate2, audio2 = wavfile.read(right_audio_path)

    assert samplerate1 == samplerate2

    logger.info("Calculating synchronization delay")
    delay = calculate_synchronization_delay(audio1, audio2, samplerate=samplerate1)
    ...
    synchronized_left_video_path = os.path.join(temp_dir, f'{generate_random_string()}.{file_type}')
    synchronized_right_video_path = os.path.join(temp_dir, f'{generate_random_string()}.{file_type}')
    synchronize_videos(video1_path=left_video_path, video2_path=right_video_path, delay=delay,
                       video1_output_path=synchronized_left_video_path, video2_output_path=synchronized_right_video_path)

    logger.info("Merging audiotracks")
    merged_audio = merge_audio_tracks(audio1, audio2)
    merged_audio_path = os.path.join(temp_dir, f'{generate_random_string()}.wav')
    ...
    if use_mixer:
        ...
        mixed_video_path = os.path.join(temp_dir, f'{generate_random_string()}.{file_type}')

        logger.info("Starting mixing")
        mixed_video_output = optical_flow_mixer.mix_video_with_field_mask(left_stream, right_stream, mixed_video_path)
    ...
    if upload_to_Youtube:
        ...
        final_video_path = os.path.join(temp_dir, f'{generate_random_string()}.{file_type}')
